...
import { 
  Play, ArrowRight, Calendar, MapPin, Clock, ExternalLink, X, ChevronRight, 
  Church, Compass, Users, BookOpen, Heart, HelpCircle, FileText, Shield, 
  UserCheck, Coins, Building2, Globe 
} from 'lucide-react';
import { Language, ActiveTab, Sermon, ChurchEvent, CarouselSlide } from '../types';
import { DICTIONARY, SERMONS, EVENTS, CAROUSEL_SLIDES } from '../data';
import { getSermons, getEvents, getCarouselSlides } from '../lib/supabase';
import DenominationLogo from './DenominationLogo';
// @ts-ignore
import heroBgImage from '../20250420_v1.jpg';
  ...
  return (
    <>
      <div id="home-view" className="animate-fade-in">
        ...
        <section id="home-hero-carousel" className="relative h-[550px] sm:h-[600px] w-full overflow-hidden bg-neutral-900">
            ...
            return (
              ...
              >
                <div
                  ...
                  style={{
                    backgroundImage: `url(${slideItem.image || heroBgImage})`
                  }}
                />
                <div className="absolute inset-0 bg-gradient-to-t from-neutral-950 via-neutral-900/60 to-black/40 z-10" />

                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-20 h-full flex flex-col justify-end pb-16 pt-32">
                  <div className="max-w-3xl space-y-4">
                    <div className="flex flex-wrap items-center gap-3">
                      <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-mono font-bold bg-amber-400 text-neutral-950 uppercase tracking-widest shadow-md">
                        <Calendar className="h-3.5 w-3.5" />
                        {typeof slideItem.tag === 'object' ? slideItem.tag[language] : slideItem.tag}
                      </span>
                      <div className="inline-flex items-center bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full shadow-md border border-white/20">
                        <DenominationLogo theme="light" className="h-4" />
                      </div>
                    </div>

                    <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black font-sans text-white tracking-tight leading-tight uppercase drop-shadow-md">
                      {typeof slideItem.title === 'object' ? slideItem.title[language] : slideItem.title}
                    </h1>

                    <p className="text-gray-200 text-sm sm:text-base leading-relaxed max-w-2xl font-sans border-l-2 border-amber-400 pl-3 line-clamp-2">
                      {typeof slideItem.description === 'object' ? slideItem.description[language] : slideItem.description}
                    </p>

                    <div className="flex flex-wrap items-center gap-4 pt-2">
                      <button
                        onClick={() => onChangeTab((slideItem.buttonLink as ActiveTab) || 'eventos')}
                        className="px-7 py-3 rounded-xl font-bold text-neutral-950 bg-amber-400 hover:bg-amber-300 transition-all shadow-lg hover:shadow-amber-400/20 hover:-translate-y-0.5 duration-150 flex items-center gap-2 cursor-pointer"
                      >
                        <span>{typeof slideItem.buttonText === 'object' ? slideItem.buttonText[language] : slideItem.buttonText}</span>
                        <ChevronRight className="h-4 w-4" />
                      </button>
                      <button
                        onClick={() => onChangeTab('cultos')}
                        className="px-7 py-3 rounded-xl font-bold text-white bg-white/10 hover:bg-white/20 transition-all border border-white/20 backdrop-blur-sm flex items-center gap-2 cursor-pointer"
                      >
          ...
          {slides.length > 1 && (
            <>
              <button
                ...
                className="absolute left-4 sm:left-8 top-1/2 -translate-y-1/2 z-30 w-11 h-11 flex items-center justify-center rounded-full bg-black/40 text-white hover:bg-amber-500 hover:text-neutral-950 transition-colors backdrop-blur-sm border border-white/10 cursor-pointer"
              >
              ...
              <button
                ...
                className="absolute right-4 sm:right-8 top-1/2 -translate-y-1/2 z-30 w-11 h-11 flex items-center justify-center rounded-full bg-black/40 text-white hover:bg-amber-500 hover:text-neutral-950 transition-colors backdrop-blur-sm border border-white/10 cursor-pointer"
              >
      ...
      {/* 2. ABOUT NAVIGATION SYSTEM (Sub-Menu Sobre a Igreja) */}
      <section id="about-navigation" className="bg-[#fcfaf5] border-t border-b border-gray-100 py-4 shadow-sm overflow-x-auto scrollbar-none">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center space-x-2 whitespace-nowrap min-w-max">
            
            <button
              onClick={() => onChangeTab('sobre', 'historia')}
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-mono font-bold uppercase tracking-wider bg-white hover:bg-amber-50 text-gray-700 border border-gray-100 hover:border-amber-200 transition-all cursor-pointer shadow-sm"
            >
            ...
            <button
              onClick={() => onChangeTab('sobre', 'confissao')}
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-mono font-bold uppercase tracking-wider bg-white hover:bg-amber-50 text-gray-700 border border-gray-100 hover:border-amber-200 transition-all cursor-pointer shadow-sm"
            >
            ...
            <button
              onClick={() => onChangeTab('sobre', 'lideranca')}
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-mono font-bold uppercase tracking-wider bg-white hover:bg-amber-50 text-gray-700 border border-gray-100 hover:border-amber-200 transition-all cursor-pointer shadow-sm"
            >
            ...
            <button
              onClick={() => onChangeTab('sobre', 'dizimos')}
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-mono font-bold uppercase tracking-wider bg-white hover:bg-amber-50 text-gray-700 border border-gray-100 hover:border-amber-200 transition-all cursor-pointer shadow-sm"
            >
            ...
            <button
              onClick={() => onChangeTab('sobre', 'denominacao')}
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-mono font-bold uppercase tracking-wider bg-white hover:bg-amber-50 text-gray-700 border border-gray-100 hover:border-amber-200 transition-all cursor-pointer shadow-sm"
            >
            ...
            <button
              onClick={() => onChangeTab('sobre', 'missionarios')}
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-mono font-bold uppercase tracking-wider bg-white hover:bg-amber-50 text-gray-700 border border-gray-100 hover:border-amber-200 transition-all cursor-pointer shadow-sm"
            >
            ...
            <button
              onClick={() => onChangeTab('sobre', 'grupos')}
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-mono font-bold uppercase tracking-wider bg-white hover:bg-amber-50 text-gray-700 border border-gray-100 hover:border-amber-200 transition-all cursor-pointer shadow-sm"
            >
            ...
            <button
              onClick={() => onChangeTab('sobre', 'ministerios')}
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-mono font-bold uppercase tracking-wider bg-white hover:bg-amber-50 text-gray-700 border border-gray-100 hover:border-amber-200 transition-all cursor-pointer shadow-sm"
            >
      ...
      {/* 2. PASTOR MESSAGE (Centered Editorial Layout) */}
      <section id="pastor-message" className="py-20 md:py-28 bg-white border-b border-gray-100">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-6">
          ...
          <div className="h-1 w-16 bg-amber-500 rounded mx-auto" />

          <div className="space-y-4 text-gray-600 leading-relaxed font-sans text-base sm:text-lg max-w-3xl mx-auto">
            <p>{dict.pastorText1}</p>
            <p>{dict.pastorText2}</p>
          </div>

          <div className="pt-6 border-t border-gray-100 flex flex-col items-center justify-center gap-1">
            <h4 className="font-sans font-bold text-xl text-gray-900">
              {dict.pastorSignature}
            </h4>
            <p className="text-xs text-amber-700 font-mono tracking-wider font-semibold uppercase">
